...
<!--										</ul>-->
										<?php if ($tk->review == "Belum Direview"){ ?>
										<span class="review-text">(Terjawab) / </span><a class="product-review" href="#"  data-toggle="modal" data-target="#waitAnswerModal">Menunggu Review</a>
										<?php } elseif ($tk->review == "Sudah Direview") { ?>
										<span class="review-text">(Telah Direview) / </span><a class="product-review" href="#" ">Selesai</a>
										<p class="price">DONE</p>
										<?php } elseif ($tk->review == "Belum Dijawab") { ?>
										<span class="review-text">(Perlu Jawaban) / </span><a class="product-review" href="#" data-toggle="modal" data-target="#jawabanModal-<?=$tk->id_task?>">Tulis Jawaban?</a>
										<?php } else { ?>
											<span class="review-text">(Perlu Jawaban) / </span><a class="product-review" href="#" data-toggle="modal" data-target="#jawabanModal-<?=$tk->id_task?>">Tulis Jawaban?</a>
										<?php } ?>
									</div>
				...
				<div class="modal-dialog" role="document">
					<div class="modal-content">
						...
						<div class="modal-body">
							<form method="POST" action="<?=base_url('C_tim/submitTask')?>" enctype="multipart/form-data">
								...
								<div class="form-group">
									...
									<textarea class="form-control" placeholder="Tulis jawaban..." rows="5" name="jawaban"><?=$tk->jawaban?></textarea><br>
									Upload File : <input type="file" name="attachment">
									<input type="hidden" name="id_task" value="<?=$tk->id_task?>">
									<input type="hidden" name="id_tim" value="<?=$this->session->all_userdata()['userdata']['id_tim']?>">
